            ...
            <div style="padding:36px 32px;">
                ...
                @if(!empty($postImage))
                    <div style="margin-top:28px;">
                        <img
                            src="{{ config('app.url') }}/{{ ltrim($postImage, '/') }}"
                            alt="{{ $postTitle }}"
                            style="width:100%; max-width:100%; border-radius:14px; display:block;"
                        />
                    </div>
                @endif

                @if(!empty($postDescription))
                    <div style="margin-top:24px; font-size:15px; line-height:1.8; color:#6b7280;">
                        {{ $postDescription }}
                    </div>
                @endif
